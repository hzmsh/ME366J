...
if __name__ == "__main__":
	...
	for c in coordinate_list:
		...
		if c[2] != 0:
			...
			# Assume the extrusion speed = 30mm/s, nozzle diameter = 
			# extru_speed = 30
			# vol_per_s = extru_speed * math.pi * nozzle_dia^2 / 4
 			
 			# Total Volume = dist traveled * nozzle cross-sectional area
			nozzle_dia = 0.8
			total_vol = dist * (math.pi*nozzle_dia^2/4)

			# syringe dx = vol / cross area of syringe
			syringe_dia = 3
			syr_dx = total_vol / (math.pi*syringe_dia^2/4)

			# stepper angle = get_screw_angle(dist, pulley_radius)
			step_ang = get_screw_angle(syr_dx, screw_pitch)
